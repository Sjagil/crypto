"""Preregistered campaign service for beta-residual mean reversion."""
...
RESIDUAL_REVERSAL_CAMPAIGN = "RESIDUAL_REVERSAL_V1"
RESIDUAL_REVERSAL_BASE_KNOWN_TRIALS = 21_337
RESIDUAL_REVERSAL_FORWARD_START = pd.Timestamp(
    "2026-07-26T00:00:00+00:00"
)
RESIDUAL_REVERSAL_PERIODS = {
...
def residual_reversal_campaign_path(settings: Settings) -> Path:
    return (
...
def _market_paths(settings: Settings) -> dict[str, Path]:
    ...
    if missing:
        raise FileNotFoundError(
            f"missing residual-reversal datasets: {missing}"
        )
    return paths
...
def _candidate_name(candidate: ResidualReversalParameters) -> str:
    entry = int(round(abs(candidate.entry_zscore) * 10))
    return (
        f"RR_B{candidate.beta_lookback}_"
        f"H{candidate.residual_horizon}_Z{entry:02d}"
    )


def _expected_plan() -> dict[str, Any]:
    candidates = residual_reversal_parameter_set()
    policy = _policy()
    return {
        "schema_version": "residual_reversal_plan_v1",
        "status": "CAMPAIGN_PLAN",
        "campaign": RESIDUAL_REVERSAL_CAMPAIGN,
        "strategy_family": RESIDUAL_REVERSAL_FAMILY,
        "engine_version": RESIDUAL_REVERSAL_ENGINE_VERSION,
        "timeframe": "1d",
        "economic_hypothesis": (
            "In an established BTC uptrend, unusually negative "
            "multi-day returns of liquid EUR altcoins after removing "
            "their strictly prior rolling BTC beta can represent a "
            "temporary idiosyncratic overshoot that mean-reverts."
        ),
        ...
        "periods": RESIDUAL_REVERSAL_PERIODS,
        "portfolio_policy": asdict(policy),
        "signal_policy": {
            "factor": (
                "ALT_LOG_RETURN_MINUS_STRICTLY_PRIOR_ROLLING_"
                "BETA_TIMES_BTC_LOG_RETURN"
            ),
            "aggregation": "THREE_OR_FIVE_CLOSED_DAILY_RETURNS",
            "standardization": (
                "STRICTLY_PRIOR_90_DAY_RESIDUAL_SHOCK_MEAN_AND_STD"
            ),
            "entry": "NEGATIVE_RESIDUAL_ZSCORE_THRESHOLD",
            "trend_filter": "BTC_CLOSE_ABOVE_CAUSAL_EMA200",
            "exit": (
                "RESIDUAL_NORMALIZATION_OR_BTC_REGIME_LOSS_OR_"
                "FIXED_MAXIMUM_HOLD"
            ),
            "execution": "SIGNAL_AT_DAILY_CLOSE_EXECUTE_NEXT_OPEN",
            "ranking": "MOST_NEGATIVE_RESIDUAL_ZSCORE_FIRST",
            "benchmark_asset": "BTC-EUR_NOT_TRADED",
            "maximum_positions": 2,
            "position_weight": 0.20,
            "cash_yield": 0.0,
        },
        "base_known_trials": RESIDUAL_REVERSAL_BASE_KNOWN_TRIALS,
        "projected_total_known_trials": (
            RESIDUAL_REVERSAL_BASE_KNOWN_TRIALS + len(candidates)
        ),
        ...
        "known_limitations": [
            ...
            "ROLLING_BETA_IS_A_LINEAR_APPROXIMATION",
            "LONG_ONLY_RESIDUAL_REVERSAL_IS_NOT_MARKET_NEUTRAL",
...
def plan_residual_reversal_campaign(
    settings: Settings,
) -> dict[str, Any]:
    ...
    if plan_path.is_file():
        ...
        for field in immutable_fields:
            if _json_ready(stored.get(field)) != _json_ready(
                ...
                raise RuntimeError(
                    f"RESIDUAL_REVERSAL_PLAN_DRIFT:{field}"
                )
    else:
...
def _preserved_forward_fields(
    ...
    *,
    ...
    validate_forward_manifest_identity(
        ...
        forward_start=RESIDUAL_REVERSAL_FORWARD_START,
    )
    return {
...
def run_residual_reversal_campaign(
    settings: Settings,
) -> dict[str, Any]:
    ...
    for candidate in candidates:
        ...
        for period, bounds in RESIDUAL_REVERSAL_PERIODS.items():
    ...
    if matrix.empty or matrix.shape[1] != len(candidates):
        raise RuntimeError(
            "RESIDUAL_REVERSAL_DEVELOPMENT_MATRIX_INVALID"
        )
    development_sharpes = {
    ...
    ranked_names = sorted(
        development_sharpes,
        key=lambda name: (-development_sharpes[name], name),
    )
    ...
    registry = ContentAddressedTrialRegistry(
        ...
        campaign_id=RESIDUAL_REVERSAL_CAMPAIGN,
    )
    ...
    total_known_trials = resolve_known_trial_count(
        ...
        local_known_trial_count=(
            RESIDUAL_REVERSAL_BASE_KNOWN_TRIALS
    ...
    economic_checks = {
        "all_periods_positive": all(
            ...
            for period in RESIDUAL_REVERSAL_PERIODS
        ),
        ...
        "strictly_prior_factor_causality": bool(
            ...
            and primary.integrity["strictly_prior_zscore_baseline"]
    ...
    forward_policy = _forward_policy(settings)
    for name, result in normal_results.items():
        ...
        source_identity = stable_hash(
            {
                ...
                "forward_start": (
                    RESIDUAL_REVERSAL_FORWARD_START.isoformat()
                ),
            },
            length=64,
        )
        ...
        observer = {
            ...
            "family": RESIDUAL_REVERSAL_CAMPAIGN,
            "policy_name": name,
        ...
        evidence = build_rotation_forward_evidence(
            ...
            minimum_rebalances=30,
            performance_policy=forward_policy,
        )
    ...
    report_path = residual_reversal_campaign_path(settings)
    payload = {
        "schema_version": "residual_reversal_report_v1",
        ...
        [
            {
                ...
                "residual_horizon": row["parameters"][
                    "residual_horizon"
                ],
