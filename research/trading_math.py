"""
research/trading_math.py

Canonical mathematics toolkit for long-only crypto spot research.
...
def calculate_position_size(
    ...
    *,
    ...
    if stop >= entry:
        raise ValueError("long-only stop_price must be below entry_price")
    distance = entry - stop

    risk_budget = equity * risk_fraction
    friction = fee + slippage
    estimated_cost_per_unit = (entry + stop) * friction
    risk_per_unit = distance + estimated_cost_per_unit
...
def calculate_position_size_from_stop_fraction(
    ...
    **kwargs: Any,
    ...
    if "side" in kwargs:
        raise ValueError("side is not configurable; active sizing is long-only spot")
