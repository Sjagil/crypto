"""Preregistered campaign service for the daily sentiment-recovery family."""
...
SENTIMENT_RECOVERY_CAMPAIGN = "SENTIMENT_RECOVERY_V1"
SENTIMENT_RECOVERY_BASE_KNOWN_TRIALS = 21_312
SENTIMENT_RECOVERY_FORWARD_START = pd.Timestamp(
    "2026-07-26T00:00:00+00:00"
)
SENTIMENT_RECOVERY_PERIODS = {
...
def sentiment_recovery_campaign_path(settings: Settings) -> Path:
    return (
...
def _paths(settings: Settings) -> tuple[dict[str, Path], Path]:
    ...
    missing = [
        str(path)
        for path in (*market_paths.values(), sentiment_path)
        if not path.is_file()
    ]
    if missing:
        raise FileNotFoundError(
            f"missing sentiment campaign datasets: {missing}"
        )
    return market_paths, sentiment_path
...
def _candidate_name(candidate: SentimentRecoveryParameters) -> str:
    return (
        f"SR_F{candidate.fear_threshold}_"
        f"D{candidate.recovery_delta}_"
        f"EMA{candidate.trend_ema_period}"
    )


def _expected_plan() -> dict[str, Any]:
    candidates = sentiment_recovery_parameter_set()
    policy = _policy()
    return {
        "schema_version": "sentiment_recovery_plan_v1",
        "status": "CAMPAIGN_PLAN",
        "campaign": SENTIMENT_RECOVERY_CAMPAIGN,
        "strategy_family": SENTIMENT_RECOVERY_FAMILY,
        "engine_version": SENTIMENT_RECOVERY_ENGINE_VERSION,
        "timeframe": "1d",
        "economic_hypothesis": (
            "A timestamped recovery from externally measured extreme "
            "crypto fear contains incremental rebound information when "
            "BTC and the fixed BTC/ETH spot basket remain in established "
            "long-horizon trends."
        ),
        ...
        "known_limitations": [
            ...
            "RETROSPECTIVE_SENTIMENT_HISTORY_HAS_NO_REVISION_VINTAGES",
...
def plan_sentiment_recovery_campaign(
    settings: Settings,
) -> dict[str, Any]:
    ...
    if plan_path.is_file():
        ...
        for field in immutable_fields:
            if _json_ready(stored.get(field)) != _json_ready(
                ...
                raise RuntimeError(
                    f"SENTIMENT_RECOVERY_PLAN_DRIFT:{field}"
                )
    else:
...
def _preserved_forward_fields(
    ...
    *,
    ...
    validate_forward_manifest_identity(
        ...
        forward_start=SENTIMENT_RECOVERY_FORWARD_START,
    )
    return {
...
def _benchmark_summary(
    ...
    *,
    ...
    if mode == "BTC_40_BUY_HOLD":
        target.loc[:, "BTC-EUR"] = 0.40
    elif mode == "BTC_ETH_20_20_BUY_HOLD":
        target.loc[:, ["BTC-EUR", "ETH-EUR"]] = 0.20
    elif mode == "BTC_ETH_EMA200_TREND":
        ema = closes.ewm(
            span=200,
            adjust=False,
            min_periods=200,
        ).mean()
        eligible = (closes > ema).mul(
            closes["BTC-EUR"] > ema["BTC-EUR"],
            axis=0,
        )
        target = eligible.astype(float) * 0.20
    elif mode != "CASH":
        raise ValueError(f"unsupported benchmark mode: {mode}")
    executed = target.shift(1).fillna(0.0).iloc[start:]
...
def run_sentiment_recovery_campaign(
    ...
    data_hashes = {
        **{
        ...
        "ALTERNATIVE_ME_FEAR_GREED": sha256_file(sentiment_path),
    }
    ...
    by_name: dict[str, SentimentRecoveryParameters] = {}
    ...
    for candidate in candidates:
        ...
        for period, bounds in SENTIMENT_RECOVERY_PERIODS.items():
    ...
    if matrix.empty or matrix.shape[1] != len(candidates):
        raise RuntimeError("SENTIMENT_RECOVERY_RETURN_MATRIX_INVALID")

    registry = ContentAddressedTrialRegistry(
        settings.paths.lab_dir
        / "strategy_registry"
        / "sentiment_recovery_v1",
        campaign_id=SENTIMENT_RECOVERY_CAMPAIGN,
    )
    ...
    stressed_periods = {
        period: sentiment_recovery_period_metrics(
    ...
    economic_checks = {
        "all_periods_positive": all(
            ...
            for period in SENTIMENT_RECOVERY_PERIODS
        ),
        ...
        "source_governance": all(
            ...
            for field in (
                "sentiment_source_timestamped",
                "sentiment_backward_only_alignment",
    ...
    source_candidate_identity = stable_hash(
        {
            ...
            "forward_start": (
                SENTIMENT_RECOVERY_FORWARD_START.isoformat()
            ),
        },
        length=64,
    )
    ...
    observer = {
        ...
        "family": SENTIMENT_RECOVERY_CAMPAIGN,
    ...
    benchmarks = {
        ...
        for mode in (
            "CASH",
            "BTC_40_BUY_HOLD",
            "BTC_ETH_20_20_BUY_HOLD",
            "BTC_ETH_EMA200_TREND",
        )
    }
    report_path = sentiment_recovery_campaign_path(settings)
    payload = {
        "schema_version": "sentiment_recovery_report_v1",
        ...
        [
            {
                ...
                "fear_threshold": row["parameters"][
                    "fear_threshold"
                ],
                "recovery_delta": row["parameters"][
                    "recovery_delta"
                ],
                "trend_ema_period": row["parameters"][
                    "trend_ema_period"
                ],
