"""Campaign service for peer-basket beta-residual mean reversion."""
...
PEER_RESIDUAL_REVERSAL_CAMPAIGN = "PEER_RESIDUAL_REVERSAL_V1"
PEER_RESIDUAL_REVERSAL_BASE_KNOWN_TRIALS = 16_911
PEER_RESIDUAL_REVERSAL_FORWARD_START = pd.Timestamp(
    "2026-07-26T00:00:00+00:00"
)
PEER_RESIDUAL_REVERSAL_PERIODS = {
...
def peer_residual_reversal_campaign_path(
    settings: Settings,
) -> Path:
    return (
...
def _market_paths(settings: Settings) -> dict[str, Path]:
    ...
    if missing:
        raise FileNotFoundError(
            f"missing peer-residual datasets: {missing}"
        )
    return paths
...
def _candidate_name(
    candidate: PeerResidualReversalParameters,
) -> str:
    return (
        f"PRR_B{candidate.beta_lookback}_"
        f"H{candidate.residual_horizon}_Z20"
    )


def _expected_plan() -> dict[str, Any]:
    candidates = peer_residual_reversal_parameter_set()
    return {
        "schema_version": "peer_residual_reversal_plan_v1",
        "status": "CAMPAIGN_PLAN",
        "campaign": PEER_RESIDUAL_REVERSAL_CAMPAIGN,
        "strategy_family": PEER_RESIDUAL_REVERSAL_FAMILY,
        "engine_version": PEER_RESIDUAL_REVERSAL_ENGINE_VERSION,
        "timeframe": "1d",
        "economic_hypothesis": (
            "In a positive BTC regime, an unusually negative return "
            "shock of one liquid altcoin after removing its strictly "
            "prior beta to the equal-weight return of the other liquid "
            "altcoins can represent temporary peer-relative "
            "dislocation that mean-reverts."
        ),
        ...
        "periods": PEER_RESIDUAL_REVERSAL_PERIODS,
        "portfolio_policy": asdict(_policy()),
        "signal_policy": {
            "factor": (
                "ALT_RETURN_MINUS_STRICTLY_PRIOR_BETA_TIMES_"
                "EQUAL_WEIGHT_OTHER_ALT_RETURN"
            ),
            "satellites": ["ETH-EUR", "SOL-EUR", "LINK-EUR"],
            "aggregation": "THREE_OR_FIVE_CLOSED_DAILY_RETURNS",
            "standardization": (
                "STRICTLY_PRIOR_90_DAY_PEER_RESIDUAL_MEAN_AND_STD"
            ),
            "entry": "MOST_NEGATIVE_ZSCORE_AT_OR_BELOW_MINUS_TWO",
        ...
        "base_known_trials": (
            PEER_RESIDUAL_REVERSAL_BASE_KNOWN_TRIALS
        ),
        ...
        "known_limitations": [
            ...
            "THREE_ASSET_PEER_BASKET_IS A SMALL CROSS_SECTION",
...
def plan_peer_residual_reversal_campaign(
    settings: Settings,
) -> dict[str, Any]:
    ...
    if plan_path.is_file():
        ...
        for field in immutable_fields:
            if _json_ready(stored.get(field)) != _json_ready(
                ...
                raise RuntimeError(
                    f"PEER_RESIDUAL_REVERSAL_PLAN_DRIFT:{field}"
                )
    else:
...
def _preserved_forward_fields(
    ...
    *,
    ...
    validate_forward_manifest_identity(
        ...
        forward_start=PEER_RESIDUAL_REVERSAL_FORWARD_START,
    )
    return {
...
def run_peer_residual_reversal_campaign(
    settings: Settings,
) -> dict[str, Any]:
    """Execute the preregistered family and persist fail-closed evidence."""

    plan = plan_peer_residual_reversal_campaign(settings)
    ...
    for candidate in candidates:
        ...
        for period, bounds in PEER_RESIDUAL_REVERSAL_PERIODS.items():
    ...
    if matrix.empty or matrix.shape[1] != len(candidates):
        raise RuntimeError(
            "PEER_RESIDUAL_REVERSAL_DEVELOPMENT_MATRIX_INVALID"
        )
    ranked_names = sorted(
        normal_results,
        key=lambda name: (
    ...
    registry = ContentAddressedTrialRegistry(
        ...
        campaign_id=PEER_RESIDUAL_REVERSAL_CAMPAIGN,
    )
    ...
    economic_checks = {
        "all_periods_positive": all(
            ...
            for period in PEER_RESIDUAL_REVERSAL_PERIODS
        ),
        ...
        "strictly_prior_factor_causality": bool(
            primary.integrity[
                "strictly_prior_peer_beta_estimation"
            ]
    ...
    for name, result in normal_results.items():
        ...
        source_identity = stable_hash(
            {
                ...
                "forward_start": (
                    PEER_RESIDUAL_REVERSAL_FORWARD_START.isoformat()
                ),
            },
            length=64,
        )
        ...
        observer = {
            ...
            "family": PEER_RESIDUAL_REVERSAL_CAMPAIGN,
            "policy_name": name,
    ...
    report_path = peer_residual_reversal_campaign_path(settings)
    payload = {
        "schema_version": "peer_residual_reversal_report_v1",
...
__all__ = [
    "PEER_RESIDUAL_REVERSAL_BASE_KNOWN_TRIALS",
    "PEER_RESIDUAL_REVERSAL_CAMPAIGN",
    "peer_residual_reversal_campaign_path",
    "plan_peer_residual_reversal_campaign",
